...
interface MaskGenerationScreenProps {
  onAcceptMask: (identity: UserIdentity) => void;
}

export function MaskGenerationScreen({ onAcceptMask }: MaskGenerationScreenProps) {
  const [identity, setIdentity] = useState<UserIdentity>(() => generateIdentity());
    ...
    <div className="min-h-screen bg-[#000000] flex flex-col items-center justify-center p-4 selection:bg-violet-500/30">
      <div className="w-full max-w-md bg-[#121212] rounded-2xl border border-zinc-800/50 p-8 shadow-2xl relative overflow-hidden text-center">
        ...
        <p className="text-sm text-zinc-400 mb-8 relative z-10">
          Sua nova máscara está pronta. Ela será válida pelas próximas 48 horas.
        </p>
        ...
        <div className="bg-[#0a0a0a] border border-zinc-800 rounded-xl p-6 mb-8 relative z-10">
          ...
          <div className={`text-2xl font-black text-zinc-100 placeholder-zinc-800 transition-opacity duration-300 ${isRolling ? 'opacity-0' : 'opacity-100'}`}>
            {identity.nickname}
          </div>
        ...
        <div className="flex flex-col gap-3 relative z-10">
          <button
            onClick={() => onAcceptMask(identity)}
            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2"
          >
            <Check size={18} />
          ...
          <button
            ...
            disabled={isRolling}
            className="w-full bg-zinc-900 hover:bg-zinc-800 text-zinc-300 font-bold py-3.5 rounded-xl transition-all active:scale-95 border border-zinc-800 flex items-center justify-center gap-2 disabled:opacity-50"
          >
