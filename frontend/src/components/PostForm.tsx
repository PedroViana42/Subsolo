  ...
  return (
    <div className="mb-8">
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <div className="brute-card rounded-2xl p-1 relative group focus-within:border-violet-500 transition-all">
          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            placeholder="O que você quer confessar hoje?"
            className="w-full bg-transparent border-none rounded-xl p-6 text-zinc-100 placeholder-zinc-700 focus:outline-none resize-none font-sans text-lg leading-relaxed min-h-[160px]"
            maxLength={maxChars}
          />
          <div className="absolute bottom-4 right-6 text-[10px] font-mono text-zinc-600 font-bold bg-black px-3 py-1 border border-zinc-800 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div className="flex gap-2">
              {TAGS.map(tag => (
                <button
                  ...
                  className={`whitespace-nowrap px-6 py-2.5 rounded-lg text-[11px] font-mono font-bold transition-all border-2 uppercase tracking-widest ${
                    selectedTag === tag 
          ...
          <div className="md:col-span-1">
            <button
              ...
              className="w-full h-full min-h-[60px] brute-card rounded-2xl flex items-center justify-center gap-3 group relative overflow-hidden transition-all disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed bg-violet-700 hover:bg-violet-600 border-violet-500 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px]"
            >
              <AnimatePresence mode="wait">
                {isPosting ? (
                  ...
                  <motion.div 
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="flex items-center gap-2 text-white font-mono font-black uppercase tracking-[0.2em] text-[12px]"
                  >
                    Postar
                    <Send size={16} />
                  </motion.div>
